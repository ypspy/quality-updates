...
import editor.routes.source as source_routes
...
def test_preview_fast_flow_auto_result_html(monkeypatch):
    ...
    monkeypatch.setattr(source_routes.threading, "Thread", ImmediateThread)
    monkeypatch.setattr(source_fetch, "fetch_url", fake_fetch_url)
    client = editor_app.app.test_client()
...
def test_preview_fast_status_includes_stage_while_pending(monkeypatch):
    ...
    monkeypatch.setattr(source_routes.threading, "Thread", NoopThread)
    client = editor_app.app.test_client()
    first = client.get("/api/source/preview_fast?url=https%3A%2F%2Fexample.com%2Fpage")
    assert first.status_code == 200
    jobs = list(editor_app._PREVIEW_JOBS.keys())
...
def test_save_fetched_writes_zip(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(editor_config, "load_config", lambda: {"downloads_folder": "downloads/z/"})
    monkeypatch.setattr(source_fetch, "fetch_url", fake_fetch_url)
...
def test_save_pdf_writes_under_configured_folder(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(editor_config, "repo_root", lambda: root)
    monkeypatch.setattr(editor_config, "load_config", lambda: {"downloads_folder": "downloads/editor_pdfs/"})
    monkeypatch.setattr(source_fetch, "fetch_url", fake_fetch_url)
...
def test_save_pdf_duplicate_name_adds_suffix(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(editor_config, "repo_root", lambda: root)
    monkeypatch.setattr(editor_config, "load_config", lambda: {"downloads_folder": "downloads/d/"})
    monkeypatch.setattr(source_fetch, "fetch_url", fake_fetch_url)
...
def test_kasb_file_proxy_saves_to_configured_downloads_folder(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(editor_config, "repo_root", lambda: root)
    monkeypatch.setattr(editor_config, "load_config", lambda: {"downloads_folder": "downloads/kasb_test/"})
    monkeypatch.setattr(source_routes.requests, "post", fake_post)
