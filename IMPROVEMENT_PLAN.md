...
**진행 현황**: 1단계(기초 및 중요 수정) 완료 (2025-01-25), 2단계(MkDocs 설정 강화) 완료 (2025-01-25), 3단계(콘텐츠 표준화) 완료 (2025-01-25), 4단계(개발 문서화) 완료 (2025-01-25)
...
**새 파일**: `.editorconfig`
- 에디터 간 일관된 포맷팅 (UTF-8, LF, trim_trailing_whitespace, insert_final_newline)
- .md, .yml, .yaml, .css, .js, .html, .json에 대한 indent_style=space, indent_size=2

**새 파일**: `.markdownlint.json`
- MD013(줄 길이) 비활성화 — 한국어 콘텐츠 대응
- MD024(중복 제목) allow_different_nesting, MD033(MD/HTML 혼용)·MD041(첫 줄) 비활성화

### 4.3 package.json 확장 ✅
- 스크립트: `dev`(mkdocs serve), `build:strict`(mkdocs build --strict), `clean`(rimraf site .cache), `lint:md`(markdownlint)
- 메타데이터: description, keywords, repository, version
- devDependencies: markdownlint-cli, rimraf 추가

**검증**: 브라우저에서 문서 읽기, IDE에서 .editorconfig 테스트

#### 4단계 완료 요약 (2025-01-25)

| 항목 | 상태 | 비고 |
|------|------|------|
| **4.1 README.md** | ✅ | 목차, 기능, 로컬 설정(venv/pip/npm), 프로젝트 구조, 프론트매터 템플릿, 배포(Render), CONTRIBUTING 링크 |
| **4.1 CONTRIBUTING.md** | ✅ | 이슈/PR, 코드 스타일(MD/YAML/CSS/JS), 커밋 규칙, 콘텐츠 절차, 테스트 체크리스트, 이미지/alt, 성능 |
| **4.2 .editorconfig** | ✅ | root=true, [*] 공통 + 파일별 indent 2 spaces |
| **4.2 .markdownlint.json** | ✅ | MD013 false, MD024 allow_different_nesting, MD033/MD041 false |
| **4.3 package.json** | ✅ | dev, build:strict, clean, lint:md 스크립트 및 markdownlint-cli, rimraf |
